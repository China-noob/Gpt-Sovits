...
def open_asr(asr_inp_dir, asr_opt_dir, asr_model, asr_model_size, asr_lang):
    ...
    if(p_asr==None):
        ...
        # return None
def open_asr_xht(asr_inp_dir, asr_opt_dir, asr_model, asr_model_size, asr_lang):
    global p_asr
    if(p_asr==None):
        asr_inp_dir=my_utils.clean_path(asr_inp_dir)
        cmd = f'"python" tools/asr/{asr_dict[asr_model]["path"]}'
        cmd += f' -i "{asr_inp_dir}"'
        cmd += f' -o "{asr_opt_dir}"'
        cmd += f' -s {asr_model_size}'
        cmd += f' -l {asr_lang}'
        ...
        print( "ASR任务开启：%s"%cmd,{"__type__":"update","visible":False},{"__type__":"update","visible":True})
        print(cmd)
        p_asr = Popen(cmd, shell=True)
        p_asr.wait()
        p_asr=None
        print( f"ASR任务完成, 查看终端进行下一步",{"__type__":"update","visible":True},{"__type__":"update","visible":False})
    else:
        print ("已有正在进行的ASR任务，需先终止才能开启下一次任务",{"__type__":"update","visible":False},{"__type__":"update","visible":True})
        # return None
def close_asr():
    global p_asr
    if(p_asr!=None):
...
def open_slice(inp,opt_root,threshold,min_length,min_interval,hop_size,max_sil_kept,_max,alpha,n_parts):
    ...
    if (ps_slice == []):
        for i_part in range(n_parts):
            cmd = '"%s" tools/slice_audio.py "%s" "%s" %s %s %s %s %s %s %s %s %s''' % ("python",inp, opt_root, threshold, min_length, min_interval, hop_size, max_sil_kept, _max, alpha, i_part, n_parts)
            print(cmd)
...
def open_slice_xht(inp,opt_root,threshold,min_length,min_interval,hop_size,max_sil_kept,_max,alpha,n_parts):
    ...
    if (ps_slice == []):
        ...
        for p in ps_slice:
            p.wait()
        ps_slice=[]
        print("切割结束",{"__type__":"update","visible":True},{"__type__":"update","visible":False})
    else:
        return
        #yield "已有正在进行的切割任务，需先终止才能开启下一次任务", {"__type__": "update", "visible": False}, {"__type__": "update", "visible": True}
def close_slice():
        ...
        try:
            ...
            if(os.path.exists(path_text)==False or (os.path.exists(path_text)==True and len(open(path_text,"r",encoding="utf8").read().strip("\n").split("\n"))<2)):
                ...
                for i_part in range(all_parts):
                    ...
                    os.environ.update(config)
                    cmd = '"%s" GPT_SoVITS/prepare_datasets/1-get-text.py'%"python"
                    print(cmd)
                    p = Popen(cmd, shell=True)
                    ps1abc.append(p)
...
def open1abc_xht(inp_text,inp_wav_dir,exp_name,gpu_numbers1a,gpu_numbers1Ba,gpu_numbers1c,bert_pretrained_dir,ssl_pretrained_dir,pretrained_s2G_path):
    global ps1abc
    inp_text = my_utils.clean_path(inp_text)
    inp_wav_dir = my_utils.clean_path(inp_wav_dir)
    if (ps1abc == []):
...
def save_gpt_latest_model(name,models_folder,outputdir):
    ...
    print(model_files)
    latest_epoch = max([int(f.split("-")[-1].split(".")[0][1:]) for f in model_files])
    for i in model_files:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n','--name', type=str, help='Name of train person')
    parser.add_argument('-i','--inputdir', type=str, help='audio path')
    parser.add_argument('-o','--outputdir', type=str, help='pth output path')
    ...
    asr_opt_dir = os.path.join(outputdir, args.name,"asr_opt")
    ## 切分设置
    slice_inp_path = inputdir
    slice_opt_root = os.path.join(outputdir, args.name,"slice_opt")
    threshold = -34
    min_length = 4000
    min_interval = 300
    hop_size = 10
    ...
    print("开始切分")
    #开始切分
    open_slice_xht(slice_inp_path,slice_opt_root,threshold,min_length,min_interval,hop_size,max_sil_kept,_max,alpha,n_process)
    print("结束切分")
    #---------------------------------------------------
    #ASR
    asr_model = "达摩 ASR (中文)"
    ...
    asr_lang = "zh"
    print("开始asr")
    open_asr_xht(slice_opt_root, asr_opt_dir, asr_model, asr_size, asr_lang)
    print("结束切分")
    #---------------------------------------------------
    #一件三联
    inp_text = os.path.join(asr_opt_dir,"slice_opt.list")
    inp_wav_dir = slice_opt_root
    ...
    pretrained_s2G = "GPT_SoVITS/pretrained_models/s2G488k.pth"
    print("开始三连")
    open1abc_xht(inp_text,inp_wav_dir,exp_name,gpu_numbers1a,gpu_numbers1Ba,gpu_numbers1c,bert_pretrained_dir,cnhubert_base_dir,pretrained_s2G)
    print("结束三连")
    #---------------------------------------------------
    #---------------------------------------------------
    #训练
    ...
    save_every_epoch1Bb = 1
    gpu_numbers1Bb = "%s" % (gpus)
    print("开始训练gpt")
    open1Bb(batch_size1Bb,total_epoch1Bb,exp_name,if_dpo,if_save_latest1Bb,if_save_every_weights1Bb,save_every_epoch1Bb,gpu_numbers1Bb,pretrained_s1)
    print("结束训练gpt")
    save_gpt_latest_model(exp_name,"GPT_weights",outputdir)
    #---------------------------------------------------
    #训练
    batch_size = 4
    total_epoch = 1
    ...
    print("结束训练vits")
    save_sovits_latest_model(exp_name,"SoVITS_weights",outputdir)
    print("success")