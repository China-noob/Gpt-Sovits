...
def handle(wav_path,text,language,prompt_wav,sweight,gweight):
    ...
    with torch.no_grad():
        gen = vc_main(
            wav_path,text,language,prompt_wav
        )
...
@app.get("/")
async def tts_endpoint(wav_path = None, text = None, language = None, prompt_wav = None,sweight= None,gweight= None):
    return handle(wav_path,text,language,prompt_wav,sweight,gweight)



if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=9882, workers=1)
