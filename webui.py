...
def generate_file(file_obj):
    ...
    # 打开复制到新路径后的文件
    with open(NewfilePath, 'w') as file_obj:
...
def train(name,input_file_path,output_file_path):
    #### 数据处理
    ## 实验名
    exp_name = name
    ## 预训练的模型路径
    ...
    asr_opt_dir = os.path.join(outputdir, name,"asr_opt")
    ## 切分设置
    slice_inp_path = inputdir
    slice_opt_root = os.path.join(outputdir, name,"slice_opt")
    threshold = -34
    min_length = 4000
    min_interval = 300
    hop_size = 10
    ...
    # asr_lang = "zh"
    # print("开始asr")
    # open_asr(slice_opt_root, asr_opt_dir, asr_model, asr_size, asr_lang)
    # print("结束切分")
    # #---------------------------------------------------
    # #一件三联
    ...
    # pretrained_s2G = "GPT_SoVITS/pretrained_models/s2G488k.pth"
    # print("开始三连")
    # open1abc(inp_text,inp_wav_dir,exp_name,gpu_numbers1a,gpu_numbers1Ba,gpu_numbers1c,bert_pretrained_dir,cnhubert_base_dir,pretrained_s2G)
    # print("结束三连")
    # #---------------------------------------------------
    # #---------------------------------------------------
    # #训练
...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem(i18n("用户自定义合成")):
            name = gr.components.Textbox(label="实验名")
            input_file_path = gr.components.Textbox(label="输入文件夹")
            output_file_path = gr.components.Textbox(label="输出文件夹")
            train_info = gr.Textbox(label=i18n("训练进程"))
            one_key_train=gr.Button(i18n("开启一键训练"), variant="primary",visible=True)
        one_key_train.click(train,[name,input_file_path,output_file_path],[train_info,one_key_train])
